...
export function ProgressBar({
  ...
  return (
    <div className="flex items-center gap-5">
      {isTextDisplayed && (
        <span className="w-10">{formatTime(currentValue)}</span>
      )}
      <div className="bg-white/20 w-full rounded relative h-1">
        ...
        {isThumbDisplayed && (
          <div
            className="w-3.5 h-3.5 bg-secondary rounded-full absolute
      ...
      {isTextDisplayed && (
        <span className="text-white/50">{formatTime(value)}</span>
      )}
    </div>
  );
}
